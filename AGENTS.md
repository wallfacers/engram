...
**Remote eval box (near-free answer/extract model)** — full-scale eval runs its answer+extract model on a **rented cloud GPU** (vllm, OpenAI-compatible) so the run is near-free (only tiny judge tokens paid). Runbook + what-changes-on-restart: [docs/operations/evaluation/remote-gpu-runbook.md](docs/operations/evaluation/remote-gpu-runbook.md). 慢跑排查手册（vllm 满载但记录冻结的病理判据 / 健康基线 ~52 答案每分钟 / 诊断命令 / 已排除项）: [docs/operations/evaluation/autodl-slow-run-troubleshooting.md](docs/operations/evaluation/autodl-slow-run-troubleshooting.md) — 任何 run 速率显著低于基线先查此手册，再归因"模型慢"。 Non-negotiables: (1) **省钱 — 空闲必停**, it's metered; never leave it idle. (2) Its **SSH host/port/password rotate every restart** and vllm may need re-launching — credentials are supplied live by the maintainer and go **only** through env/tunnel, **never** into a tracked file, log, or tool response. (3) The box is eval infra only; the engine stays local-first/offline and never depends on it.
