...
func TestCmdCorePlanReceiptClosedSemantics(t *testing.T) {
	...
	mutations := []struct {
		...
		{"no hosts", func(p *CoreExecutionPlanReceipt) { p.Hosts = nil }, "hosts empty"},
		{"unknown host", func(p *CoreExecutionPlanReceipt) { p.Hosts[0] = "terminal" }, "not a formal host"},
...
func TestPurposeOfficialDualVsDevComparison(t *testing.T) {
	...
	mutations := []mutation{
		...
		{"no hosts", true, func(m *FormalSeriesManifest) { m.Hosts = nil }, "hosts empty"},
		{"unknown host", true, func(m *FormalSeriesManifest) { m.Hosts[0] = "terminal" }, "not a formal host"},
