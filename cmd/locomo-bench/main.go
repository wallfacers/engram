...
func validateMechanismArms(opt options) error {
	...
	// removed so compiler/representation treatment manifests can freeze.
...
// hasClusterSweepHit reports whether any retrieved hit carries the engine's
// cluster-sweep marker (engine-side sweep, kept after the --cluster-sweep flag
// was removed by 044; the engine can still emit the marker).
