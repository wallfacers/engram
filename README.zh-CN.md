...
从 ~3,614 token 降到 ~441 token（约 1/8），正确率方向非负（85.6% vs 84.9%，单次，
repeats 确认中），类别无回落。由于 token 节省在任何正确率差值下都成立，这是上面
"预算驱动 +3.20pp"的预算高效对应物——预算对齐视角下第一个"更少 token、更多信号"
的结果。该阶段保持 opt-in、默认关闭。
