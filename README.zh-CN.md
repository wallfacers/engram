...
| LoCoMo（1,540） | 450-char · k150 · q90 | Qwen3.8-27B | **87.99%** | 1-rep clean · 较 k30·q28 +2.01pp（p=.011）· [047 verdict](docs/evaluation/reports/047-full-run-interrupted-2026-08-20.md) |
...
各行统一：unified 答题契约、deepseek-v4-flash 判题、clean 口径（只判 final answer）、三次配对多数投票；450-char 行为单次 clean（三次多数投票未做）。Qwen3.6-35B 即 Qwen3.6-35B-A3B-FP8；未标 quota 的行是 chunk-quota 机制引入前的配方。91.10% 行的全量 k150 等价值已在 Qwen3.6 上实测，同为 91.10%。
