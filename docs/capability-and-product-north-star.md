...
## 2. 当前诚实水位(2026-07,post 007/008;**分数正本 = [results-matrix-2026-07-26.md](./results-matrix-2026-07-26.md)**)

**LoCoMo 全量 1540 端到端 overall = 85.71%**(mem0-aligned judge=deepseek-v4-flash /
本地 Qwen3.6-35B-A3B-FP8 答题 / **bge-large** 检索 / top-k30 / 无 reranker / 3 跑多数),
换 **deepseek-v4-pro 答题**则 **89.03%**。另有 LongMemEval-S(cleaned)**抽样 100 题**
的独立基线 **80 / 85**(同上两个答题模型)——**不是全量 500,不得简称 LongMemEval**。

> 📌 **83.70% 已退役**(它是 bge-small 单 run 的旧参考点)。任何对外引用一律用上面两个数,
> 并**连同四元组(数据集 × 答题模型 × 判题模型 × 配方)一起给出**——每个数字都是这个四元组
> 的函数,不是"engram 的分"。跨行比较只在恰好一个轴不同时才有效。

**关键认知修正**:旧的 50.7% / ~74 是**判题口径伪影**(strict judge + luna 栈)。对齐 Mem0 判题口径(007)后,**差距远比想象小**:

| | 分数 | 对 engram 差距 |
|---|---:|---:|
| **engram(现参考点,同栈)** | **85.71%** | — |
| ~~MemOS(local,开源)~~ | ~~88.83~~ | ~~**~5.1pp**~~ **已作废,见下(同栈 82.40,engram 反超 3.31pp)** |
| Mem0 | 92.5 | ~~~8.8pp~~ **未做同栈剥离,仍是未知数,不得引用该差距值** |

> 🚨 **2026-07-26 二次修正 —— MemOS 那一行是第二个 regime 伪影。** 用 MemOS **自家代码**
> 跑在 engram 同款答题模型 + 同款 embedder + 同一 judge 上,**MemOS = 82.40%**,
> **低于** engram 同口径(`009-full-A-base` bge-large 3-rep 多数)**85.71%** 达 **3.31pp**。
> leaderboard 88.83 → 同栈 82.40 的 **−6.43pp 全是 answerer 强度 + judge 宽松度**。
> ⇒ **"engram 落后 MemOS ~5pp"不成立;在可比条件下 engram 领先。**
> 全表/口径诚实项(最要紧的是两边默认上下文预算差 ~3 倍、MemOS 单次答题无误差带、
> 未做配对 McNemar)见 [`memos-inhouse-locomo-repro.md §6`](./memos-inhouse-locomo-repro.md)。
>
> **认知含义**:本节开头那句"旧的 50.7%/~74 是判题口径伪影"现在要再推一层——
> **对手的分数同样带 regime 伪影。跨系统比分在拉平模型栈之前一律不可信,自家的和别家的都是。**
...
1. **分数前提变了**:当时 ~74 vs 92.5(~18pp);现 **85.71% 同栈领先 MemOS 3.31pp**(其 88.83 含 6.43pp regime 伪影),对 Mem0 的差距未剥离。"胜负不取决于外壳"依然成立,而"分数差距大到不值得做产品"**已彻底不成立**。
   → **2026-07-26 再更新**:同栈复现后对 MemOS 是 **85.71 vs 82.40(engram 领先 3.31pp)**,
   本条论据只增不减(对 Mem0 的 92.5 仍未剥离,那一支保持未知)。
...
| 里程碑 | 目标 | 门槛 |
|---|---|---|
| ~~**M1 拉平**~~ → **M1′ 守住并坐实领先** | ~~端到端对齐 MemOS 88.83(约 +5pp);先做 P0 实体图 + temporal 检索侧时间结构化~~ **目标与手段双双失效(2026-07-27 修正)**:目标锚 88.83 含 6.43pp regime 伪影、同栈下 engram 已领先 3.31pp;两条 P0 手段也都已 NO-GO(实体图=spec 014、时间窗=spec 013)。**改为**:① 零 token 的 1540 题配对 McNemar 把 +3.31pp 从点估计升为显著;② 等上下文预算重跑,剥离"engram 喂 3 倍 token"这个最大残留混淆;③ 剩余三个未验杠杆(open-domain 精准浮现 / 确定性日期脚手架 / image-captions)按台账上限逐个过端到端门 | 死规则 + 引擎不可改 + 评测门;**任何增益不得与已计入基线的口径改动叠加** |
