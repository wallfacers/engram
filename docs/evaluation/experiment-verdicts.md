...
| 030 | mechanism-go-opt-in | **读侧证据装配**（post-retrieval evidence mediation，84 题 = 029 实际子集 × 3 reps majority，Qwen3.6 + bge-large + DeepSeek mem0-aligned judge）：**装配器 chunk-first 排序显著优于 legacy 单次路径**——keep **47.6% vs base 27.4%（p=0.0455）**；trace 引用链 sidecar **50.0% vs base 27.4%（p=0.0017 显著）但 vs 装配器 keep 不显著（p=0.152）**；base-slim 控制（~688 tok）仍 27.4% 排除 token-削减伪赢。**拆解**：有效杠杆是「证据精炼」（chunk 优先 + cap 内聚焦），trace 引用链无独立于装配器的增量（evidence_count 几乎全 1）；US3 条件压缩保守 PASS（cons 41.7% vs keep 47.6%，p=1.0，consolidate 在 cap 截断下基本未触发）。**读侧结构首次实证赢**（区别于写侧 027/028、检索侧 029 证伪）。缺口：US3 精确 tokenizer 未启用（缺 `--counter-fingerprint`）。 | `--trace-mediation` / `--consolidate` default-off（SC-004）；装配器排序方向为后续读侧装配接线提供依据，不改默认路径。 | [spec 030](../../specs/030-evidence-mediation/spec.md) · [整体 verdict](reports/030-evidence-mediation-verdict.md) · [US2 配对](../../specs/030-evidence-mediation/diagnosis/us2-verdict.md) · [US3 配对](../../specs/030-evidence-mediation/diagnosis/us3-verdict.md) |
