...
| **E-MEMOSPAR** | MemOS 自家代码跑 engram 同栈（同一 vllm Qwen3.6-35B-A3B-FP8 答题 + 同一 bge-large embedder + 同一 `judgeMem0AlignedSystemPrompt` × deepseek-v4-flash × 3 judge reps）= **82.40%**（1269/1540）；分类别 multi-hop 89.36 / temporal 82.55 / open-domain 59.38 / single-hop 82.64。对同口径 engram 85.71%，Δ = **engram +3.31pp**。对该系统自报 leaderboard 88.83，**落差 −6.43pp** | [`memos-inhouse-locomo-repro.md` §6](./memos-inhouse-locomo-repro.md) + HF `wallfacers/engram-locomo-artifacts` 下 `memos-parity/`，本地镜像 `.locomo-run/memos-parity/`（含 1,540 题逐题 votes + MemOS 原始答案与 `search_context`） | **A-: 当前可复算一手数据，且是本文唯一的跨系统 apples-to-apples 测量**。硬边界三条：(1) MemOS 侧仅 **1 次答题** run（engram 侧是 3-rep 多数），无误差带；(2) **未做 1,540 题配对 McNemar**，只有两个独立点估计；(3) 两边各用自家默认检索预算，engram 实测喂 answerer 3,262 tok/次 vs MemOS ~1,059 tok/次（≈3 倍），**+3.31pp 中"上下文更多"的贡献未剥离**。⇒ 可声明"leaderboard 与同栈分之间存在 6.43pp 的 regime 落差"，**不可**声明"engram 记忆机制优于 MemOS" |
...
- **当前状态（2026-07-26 更新，016 完成）**:已有 engram 一手结果，全文见
  [specs/016-longmemeval-crossbench/verdict.md](../specs/016-longmemeval-crossbench/verdict.md)。
  数据为 **LongMemEval-oracle 全 500 题** + **LongMemEval-S (cleaned) 分层子集
  100 题**（**不是全量 S**），答题/抽取 `Qwen/Qwen3.6-35B-A3B-FP8`，判分
  `deepseek-v4-flash`。三条子结论的复现情况**不一致，必须分开讲**：

  | 子结论 | 状态 | 依据 |
  |---|---|---|
  | 「coverage 不保证 answer」 | **定性复现** | 全覆盖桶 73 题仍错 12 题；答题侧当量是检索侧的 4 倍（ORACLE 臂 86 vs 2.19，约 39 倍） |
  | 「多跑才足以判断小改进」 | **复现** | 同一配置 3 次独立运行 70/75/75，摆幅 5pp，已达 T037 两臂差（1.0pp）的 5 倍 |
  | 「协议必须完整披露」 | **复现且被加强** | 见下方向量门事件 |

  **但预注册的定量判据落空。** 判据（测量前固化，SHA
  `2142f722…09ba`）要求条件增益 ∈ [20, 50] pp 才算复现；实测 **50.2283 pp**，
  落进有意留白的 (50, 60] 空档 ⇒ **无法判定**。零覆盖桶 n=6 < 20 亦标不可判。
  距入区间仅差 0.2283 pp（该桶多 1 题答对即翻转），**未作任何补救**。
  论文中**不得**把它写成「已复现」，也不得写成「证伪」。[P-EXP]

- **本次新增的一条独立证据（可直接进 §2 regime 清单）**:S 臂建库后
  102,578 条目中 **20,503 条无向量**——`DefaultEmbedBuffer = 256` 而单库
  `Backfill` 入队 345–525，溢出被**静默丢弃**，检索静默降级、无任何报错。
  同一 store 修复前后严格证据覆盖率 **0.425 → 0.849**。这是一个纯基础设施缺陷
  伪造出算法结论的实例：不修就会得到「检索侧是瓶颈」的相反结论。**「存储层向量
  完整性」应当作为一条独立的必报 regime 轴写入报告规范**，与 answerer / judge /
  force-answer 并列。
