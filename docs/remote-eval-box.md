...
> ✅ **无人值守时可以自己停机**(2026-07-26 实测)。此前默认"只能维护者去 AutoDL
> 控制台停",于是维护者离场后跑完的机器会空转烧钱。实测 **SSH 内 `shutdown -h now`
> 有效**:命令一发 SSH 会话立即断开,随后无法重连。
>
> ```bash
> # 跑完最后一步后立即执行(先停 vllm 释放显存,再关机)
> ssh -p $BOX_PORT root@$BOX_HOST 'pkill -f vllm; sleep 2; shutdown -h now'
> ```
>
