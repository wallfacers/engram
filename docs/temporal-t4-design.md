...
> 📌 **状态已回填(2026-07-27)**:T-4 走完 spec 013 判 **NO-GO**;T-2 / T-3 的**机制**
> 已被 [`specs/017-temporal-date-scaffold/`](../specs/017-temporal-date-scaffold/) 收编为
> 一个方案(确定性日期脚手架 / TIMELINE 块)——T-3 的"按时间排序 + 结构化日期呈现"= 脚手架的
> 排序编号,T-2 的"区间/时长算术"= 脚手架预算好的时间跨度。**关键差别**:017 **不**依赖
> "引擎把 range 暴露给 reader",而是在**答题上下文构造阶段用确定性代码**从已有 `[event:]`
> 日期算好——因此**引擎零改**,规避了本文 T-2 那条"依赖引擎暴露"的路径。
> 另需注意:纯 prompt 让模型自己做这些推理的路已在 **spec 014 两连败**(强化契约反而更差),
> 故 017 走"代码替代模型能力",不走"更强的契约措辞"。

| 杠杆 | 治哪类错题 | 层 | 状态 |
|---|---|---|---|
| **T-2** 区间/时长算术(依赖 range 暴露给 reader) | 4 时长算术 | 答题(+引擎暴露) | ~~待立项~~ → **机制并入 spec 017**(改为确定性代码预算,不依赖引擎暴露) |
| **T-3** 候选日期消歧展示(按时间排序 + 结构化日期) | 19 候选选择大头 | 检索+答题 | ~~待立项~~ → **机制并入 spec 017**(TIMELINE 块排序编号) |
| **T-4** 第 4 路时间融合 RRF | ≤16 检索排序 | 检索 | ~~本文,contract-first GO / 出货 NO-GO~~ → **spec 013 实测 NO-GO**(解析器仅 19.6–29.6% 点火;temporal `oracle_lift@30`=0.000) |
