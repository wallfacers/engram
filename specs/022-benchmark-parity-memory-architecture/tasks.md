...
- [ ] T113 [US3/repivot] 为「chunk→紧凑 turn-granularity」表示探索加 store-build-time chunk-size flag：`cmd/locomo-bench/chunks.go` 把 `chunkTargetChars`/`chunkMaxChars` 从 `const` 改 `var`，`cmd/locomo-bench/main.go` 加 `--chunk-target-chars`/`--chunk-max-chars`（默认 900/1100 不变、向后兼容），使同一 binary 能建不同粒度 store 做 022 Increment 2 representation bake-off。前置门 A0（`--chunk-quota 0` 纯 fact e2e，复用 022-full-store）已证伪 chunk→pure-fact 直接替换：73.70%（−10.13pp，single-hop −16pp），根因是 ADD-only fact 的 turn 覆盖 < verbatim chunk（compiler-arm extractive 已把每 hit 展开为 raw_turn Evidence span，精度在，非精度问题）；token 降到 1529（−58%，1.44× MemOS）。门 B（chunk=150 coverage sweep over quota=0/12/30，建新 store 复用）验证 turn 粒度能否在 ~1k token 保 turn recall ≥ 0.808 基线。engine 未触碰（仅 eval-harness flag）
...
- [X] T114 为 representation/compiler/event/gap/projection treatment 实现可冻结、可重放且 fail-closed 的 formal protocol：manifest 必须记录真实 `stage`、`arm`、mechanism flags 与 B1 `control_protocol_hash`，Compiler 四臂必须逐字节读取同一 `candidate-replay`（包含 exact-token arm、post-freeze retrieval=0），runner 必须拒绝 option/manifest/candidate digest 漂移；补齐 CLI、artifact validator 和集成测试到 `cmd/locomo-bench/eval_protocol.go`、`eval_runner.go`、`eval_artifact.go`、`main.go` 及对应 tests，满足 FR-003、FR-014、FR-036、FR-040、SC-005、SC-006、SC-008、SC-014（四臂 replay 框架与 exact-token arm 就位；planner arm 属 T070）
- [X] T115 实现可运行的 judge-audit prepare/finalize workflow：从 control/treatment 三次结果确定性生成全 discordant + 分层 concordant 的盲化 reviewer packets，私有 arm/raw-label key 分离保存，严格导入两位独立 reviewer 与必要 adjudication，输出 raw/corrected FN/FP/agreement/verdict-change summary 并绑定 protocol/artifact hashes；接入 `cmd/locomo-bench` CLI、离线 tests 和 `cmd/locomo-bench/testdata/022/README.md`，满足 FR-037、FR-041、SC-013、SC-015
