...
| D2 | 每子查询精检多深(候选深度 D) | 每子查询用 `SearchWithDiagnostics(ctx, sub, k)`,复用引擎**已有的内部候选池** `pool = k*candidateMultiplier`(min `minCandidatePool`)作为 L_i 的深度来源;最终融合后仍 **top-k 截断** | 单查询本就在内部用 `k*candidateMultiplier` 的候选池(retriever.go:140)。多查询沿用同一深度 = 不新增预算;**最终喂答题的仍是 k=30**,深度只是内部融合候选宇宙 | 「每子查询也只取 top-k」——否:gold 若在某子查询 rank∈(k, pool] 会被过早截断,融合捞不回来 |
