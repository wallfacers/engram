...
- [ ] T011 [US1] 实现 `memory_search` handler 于 `mcpserver/tools.go`:调 `Retriever.Search(ctx,query,limit)`,映射 `Result`→输出(snippet 由 content 派生),含 `degraded` 字段(值由 T016 落实)。产物:`mcpserver/tools.go`。
...
- [ ] T016 [US2] 落实降级标注:`memory_search` 依 Registry 的 `embClient==nil` 结构化设 `degraded.semantic` 与 `reason="no embedding endpoint configured (offline mode)"`(research R3 诚实口径);确认 write/list/get/delete 不依赖任何端点。使 T015 通过。产物:`mcpserver/tools.go`、`mcpserver/registry.go`(暴露 embClient 状态)。
