...
- **评测侧隔离(H1/research D7)**: baseline/treatment 都在 009 canonical 店的 run-local 副本上 Backfill;baseline 副本随后剥离影子,treatment 副本保留。canonical 绝不作为运行店打开/写入,两臂唯一变量=影子向量。
...
- **字段**: 配对端到端里 baseline 副本(剥离影子)vs treatment 副本(保留影子)的 `answer_context_tokens`、`final_top_k`、`subquery_count`(=1,本 feature 不分解)。
...
- **字段**: 每题 `gold_has_alias`(该题 gold fact 是否有 alias)、`baseline_gold_rank`/`treatment_gold_rank`、`gold_entered/left_top_30`、`coverage_at_30_delta`;按 `gold_has_alias` 分层汇总。
- **判定**: **主看「gold 有 alias」子层** gold 是否净升 top-30(全局会被 48% 无 alias 层稀释);coverage 仅诊断不作 verdict;子层不升即 NO-GO 止损。落 `.locomo-run/011-recall/`。
- **符号口径**(承 010 勘误):`rank_delta=treatment-baseline`(负=前移/变好),`coverage_delta=treatment-baseline`(正=变好),避免 010 式符号歧义。
