...
- **SC-003 (FREE GATE — the cost firewall)**: **GO** if and only if some signal (typed-claim, confidence, or combination) has an operating point with **adversarial-recall ≥ 40%** AND **answerable-false-abstain ≤ 5%** — equivalently, a net of **≥ +100 questions** over force-answer across the 1986-question set at that point (gain = 446 × recall; loss = 1540 × false-abstain × 0.676 baseline answerable accuracy). Otherwise **NO-GO**, and no paid answering occurs.
