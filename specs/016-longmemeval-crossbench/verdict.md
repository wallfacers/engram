...
## US5 · 召回深度 / 上下文预算 bracket —— 双侧探尽,无杠杆(2026-07-28)

**问题**:LongMemEval 的 haystack 是 LoCoMo 的 ~50×(500 session/题 vs 10 段对话),
LoCoMo 调的 `top-k 30 / chunk-quota 12` 在这个干扰密度下还是不是最优?往**深**了加
(更多召回)或往**浅**了砍(更聚焦的上下文)有没有得分?

### 前置:attribution 定位瓶颈(retrieval-only,0 答题/判题)

全 500 题跑 `--attribution-trace --wide-pool 800`(60s),与 3-rep 多数票 join:

| 96 道错题 | 题数 | 占比 |
|---|---:|---:|
| **answer-side**(gold 已进 top-30,答题器拿到了) | **84** | **87.5%** |
| 可救(gold 在 800 池内但 rank>30) | 7 | 上限 1.40pp |
| 不可达(不在 800 宽池) | 5 | — |

那 84 道 answer-side 错题的 **gold rank 中位 = 2**(p25=1, p75=4, 81% 在前 5 名)。
**multi-session**(最看好的候选)34 道错题里 32 道(94%)gold 已在 top-30、rank 中位 2。
⇒ 检索侧 comprehensively solved:证据顶在答题器第 2 条位置,它仍答不对。加召回深度
对这些题零帮助(gold 已经在),与 LoCoMo 的 008 铁律同向、且更一边倒。

### bracket:top-k 15 / 30 / 40 / 50(各 3-rep 多数票,配对 McNemar vs k30ref)

| top-k | chunk-quota | maj acc | Δ vs k30 | McNemar χ² | 判定 |
|---:|---:|---:|---:|---:|---|
| 15 | 6 | 77.80% | −1.0pp | 0.45 | ns |
| **30** | **12** | **78.80%** | — | — | baseline(昨夜 80.80%) |
| 40 | 16 | 80.00% | +1.2pp | 0.69 | ns |
| 50 | 20 | 80.40% | +1.6pp | 1.39 | ns |

**整条 bracket 在噪声里是平的**(全部 ns,最大 χ²=1.39 ≪ 3.841)。有一个微弱单调
上升趋势(k15→k50: 77.8→78.8→80.0→80.4),但每个两两对比都不显著,总跨度 2.6pp
刚好压在噪声标尺 2.0pp(main 80.80 vs k30ref 78.80)上,处于无法确信区;且增量在
缩小(+1.0/+1.2/+0.4),已近平台。k50 比 k30 多 67% 上下文换 +1.6pp(ns),真实部署
不值。

**结论:召回深度/上下文预算在 15–50 区间对答分没有可测影响。k30 不是唯一甜点,而是
整片平台的一例。** 检索侧(gold rank 2)+ 上下文预算侧(bracket 平)双侧探尽,
**80.80% 是 Qwen3.6-35B 在 LongMemEval 的天花板**,瓶颈是答题器的跨 session 综合能力
(answer-side 87.5%),非 engram 检索/上下文。唯一能动的真杠杆是换更强 answerer,
已因成本否决(v4-pro 太贵)。

### 方法论教训:别在 run 写完前读多数票

本节 k15 的首个读数是 **70.60%**(误报"暴死 −8.2pp"),后纠正确认是 **77.80%**。
根因:首个 inline 分析在 k15 的 run-3 判题 jsonl **还在写**时就跑了——部分文件里,
