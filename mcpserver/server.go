...
func registerTools(server *mcp.Server, adapter *toolAdapter) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Upsert one memory entry by name in a namespace; an existing entry with the same name is replaced. " +
			...
			"this tool, not those, is the memory system. Never store secrets, transient one-off task details, this-week scheduling plans, or a conditional what-if " +
			"(\"if I switch to a Mac next month…\" is hypothetical, not a standing fact or a decision). When declining to store a secret, write nothing derived from that same request — " +
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Return a ranked bounded subset of relevant memories from one namespace; empty results do not prove absence. " +
			...
			"and BEFORE performing actions (install, build, run, deploy, format, commit, history rewrite) that a remembered convention (package manager, toolchain, naming, branch policy) could govern: a plain task phrasing (\"install the dependencies\", \"rewrite these commits\") does not make it memory-independent. " +
			...
			"Do NOT search only when nothing remembered could govern the reply: a general technical question, comparison, or how-to with no reference to the user's own setup, past choices, or habits (best practices, framework suitability, generic error handling, math, definitions); " +
			"an imperative command whose action and target are already fully specified — \"remember to <do X>\" with exact parameters given (记得把端口 8080 改成 9090 并提交 / remember to rename the staging branch to release/2.1) — is a reminder to act, not a memory lookup, even when it touches git or config files; questions about the current environment's contents (which files are in this directory, what is running now) — answered from the filesystem; and plain environment/tool state work (browser/system cache, Redis or DB cache tuning, IDE window layout, bookmarks, Notion links). " +
			"A request that must CONSULT a convention before acting — installing or building \"the way I usually do\", rewriting commits under a branch/history policy, choosing names — still searches first: when the user has not supplied the how, memory is where the how lives.",
